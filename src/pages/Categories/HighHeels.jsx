...
function Clothes() {
   ...
   useEffect(() => {
      const fetchData = async () => {
         const result = await axios('https://630ed147379256341881df89.mockapi.io/products?filter=high');
         setData(result.data);
      };
      fetchData();
   }, []);
      ...
      <>
         ...
         <div className="w-full h-[140px] relative bg-[#f6f6f6] mt-[32px] selection-default">
            <div className="absolute top-[40%] left-[4%] store-menu-route">
               ...
               <span className="mx-[10px]">/</span>
               <Link className="" to="/store">
                  <span className="">High heels</span>
               </Link>
            </div>
            <div className="absolute top-[48%] left-[4%] mt-[10px]">
               <span className="store-big-title">Category: high heels</span>
            </div>
         </div>
